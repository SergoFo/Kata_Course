public class pz_3_2_6__Robot_getConnection_try_catch {
    ...
    public static class Robot implements RobotConnection {
        ...
        public void moveRobotTo(int toX, int toY) {         // Перемещает в координаты
            int dx = toX - getX();
            int dy = toY - getY();
    ...
    public interface RobotConnection extends AutoCloseable {                       // Для движения и закрытия
        // Может бросить непроверяемое исключение RobotConnectionException !!!
        void moveRobotTo(int x, int y);     // Перемещает в координаты

        @Override
        void close();      // Закрывает соединение
    }

    public interface RobotConnectionManager {                                      // Для создания соединения
        // Может бросить непроверяемое исключение RobotConnectionException !!!
        RobotConnection getConnection();
    }

    public static class RobotConnectionException extends RuntimeException {         // Исключение непроверяемое
